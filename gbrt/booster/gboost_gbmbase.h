 ...
 *        this class can be used as base code to create booster variants
 *
 *  relation to gboost.h
 *  (1) gboost.h provides a interface to a single booster(e.g. a single regression tree )
...
class GBMBaseModel{
 public:
  ...
  struct Param{
    ...
    int num_feature;
    /** \brief size of prediction buffer allocated for buffering boosting computation */
    int num_pbuffer;
    /**
 ...
 public:
   ...
   */
  inline void SetParam(const char *name, const char *val) {
    if(strncmp(name, "bst:", 4) == 0) { cfg.PushBack(name+4, val); }
  ...
  inline void SaveModel(utils::IStream &fo) const {
    ...
    for (size_t i = 0; i < boosters.size(); ++i) { boosters[i]->SaveModel(fo); }
    if(booster_info.size() != 0) {
      fo.Write(&booster_info[0], pred_buffer.size()*sizeof(float));
    }
    if(param.num_pbuffer != 0) {
 ...
 protected:
  ...
  inline void FreeSpace(void) {
    for(size_t i = 0; i < boosters.size(); ++i) { delete boosters[i]; }
  ...
  inline void ConfigBooster(booster::IBooster *bst) {
    cfg.BeforeFirst();
    while(cfg.Next()) { bst->SetParam(cfg.name(), cfg.val()); }
  }
  /**
  ...
  inline booster::IBooster *GetUpdateBooster(void) {
    ...
    } else { this->ConfigBooster(boosters.back()); }
